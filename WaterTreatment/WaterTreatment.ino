...
{ //const char *pcTaskName = "Web server is running\r\n";
	...
	for(;;)
	{
		...
		if(MC.get_errcode() == OK) {
			...
			} else if((((MC.NextRegenAfterDays == 0 || MC.NextRegenSoftAfterDays == 0)
						&& (SAM_RTC_HOUR(RTC->RTC_TIMR) >= PIN_LED_SRV_INFO_NEXT_REGEN_BEGIN_HOUR || MC.get_NeedRegen() || MC.get_NeedRegenSoftening()))
...
{ //const char *pcTaskName = "ReadSensor\r\n";
	...
	#endif
		uint32_t passed;
		bool RO_was_flow = MC.sFrequency[REVERSE_OSMOS_FC].Read();
		if(RO_was_flow) {
			{
...
//#endif
		...
		static uint32_t tmp;
		if(GetTickCount() - ttime < cDELAY_DS1820 - cDELAY_DS1820/10) {
				...
				if(GETBIT(MC.Option.flags2, fSepticPumpRelay) && (SepticPumpRelayStatus == MODBUS_RELAY_CMD_ON || SepticPumpRelayStatus == MODBUS_RELAY_CMD_OFF)) {
					...
					if(Modbus.RelaySwitch(MODBUS_SEPTIC_PUMP_RELAY_ADDR, MODBUS_SEPTIC_PUMP_RELAY_ID,
						...
						if(SepticPumpRelayStatus == MODBUS_RELAY_CMD_OFF) {
							...
							SepticPumpRelayStatus = MODBUS_RELAY_CMD_ON;	// Pulse 1 sec
	#else
							SepticPumpRelayStatus = MODBUS_RELAY_OFF;
	#endif
						} else {
...
#endif
				{
					...
					if(!GETBIT(MC.Option.flags2, fCheckSeptic)) SepticPower = 0;
						...
						if(err == OK) {
							...
							uint16_t pwr_prev = SepticPower;
							SepticPower = tmp;
							if(SepticPowerMax < tmp) SepticPowerMax = tmp;
							uint32_t minp = MC.Option.SepticPumpMinPower * 10;
							if(tmp >= minp) { // работает
							...
							PumpReadCounter = MC.Option.PumpReadPeriod * 1000 / TIME_READ_SENSOR / 2 - 1;
#endif
							SepticErrors++;
								...
								if(tmp >= dpmp) { // работает
									...
									if(ut - DrainPumpTimeLast >= MC.Option.PumpStartTime) {
										if(MC.Option.DrainPumpMaxPower && tmp > MC.Option.DrainPumpMaxPower) {
											journal.jprintf("Drain pump: %.3dA\n", tmp);
											DrainPumpPower = tmp;
		...
		if(OneWire_time == ttime) { // Был запрос датчиков температуры
		...
		if(WaterBoosterStatus == 0) {
			if(!CriticalErrors && press != ERROR_PRESS
				...
				if(!LowConsumeMode || (!MC.dRelay[RFEEDPUMP].get_Relay() && AfterFilledTimer == 0)) {
					...
					if(l < MC.Option.MinWaterBoosterCountL && !MC.Osmos_PWATER_Added && (LCD_setup & 0xFF00) != LCD_SetupMenu_FlowCheck) {
						...
						if(!reg_active) {
							if(!MC.Osmos_PWATER_Added && MC.Osmos_PWATER_Cnt <= MC.Option.PWATER_Osmos_Step) {
								...
								if(++MC.Osmos_PWATER_BoosterMax_cnt == BOOSTERMAX_HIST_MAX) {
									...
									if(MC.Osmos_PWATER_BoosterMax < MC.Option.WaterBoosterMinTank) {
										MC.Osmos_PWATER_BoosterMax = MC.Option.WaterBoosterMinTank;
										vPumpsNewError = ERR_LOW_BOOSTER_TANK;
									}
