...
void clientMQTT::initMQTT(uint8_t web_task)
{
 ...
 strcpy(mqttSettintg.mqtt_id,"WaterTreatment");                 // Идентификатор клиента на MQTT сервере
 // NARMON
 ...
 strcpy(mqttSettintg.narodMon_id,"WaterTreatment");             // Идентификатор клиента на MQTT сервере
...
{
 if (writeEEPROM_I2C(adr, (byte*)&mqttSettintg, sizeof(mqttSettintg))) { set_Error(ERR_SAVE_EEPROM,(char*)nameMainClass); return ERR_SAVE_EEPROM;}  adr=adr+sizeof(mqttSettintg);           // записать параметры MQTT
 return adr;
}
...
{
  if (readEEPROM_I2C(adr, (byte*)&mqttSettintg, sizeof(mqttSettintg))) { set_Error(ERR_LOAD_EEPROM,(char*)nameMainClass); return ERR_LOAD_EEPROM;}  adr=adr+sizeof(mqttSettintg);           // прочитать параметры MQTT
  return adr;
}
...
if (NM) // В зависимости от того с кем надо соединяться
         ...
         w5200_MQTT.connect(MC.get_netMAC(), mqttSettintg.narodMon_login,mqttSettintg.narodMon_password);       // Соедиенение с народным мониторингом
         #ifdef MQTT_REPEAT            // разрешен повтор соединения
         if (!w5200_MQTT.connected()) { _delay(20); ShowSockRegisters(W5200_SOCK_SYS); w5200_MQTT.connect(MC.get_netMAC(), mqttSettintg.narodMon_login,mqttSettintg.narodMon_password); } // вторая попытка
         #endif
         }
         else
         {
          if(MC.clMQTT.get_TSUse())
               {
