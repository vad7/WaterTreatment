...
//  Arduino DUE Core
    ...
    #define INDEX_FILE			"plan.html"       // стартовый файл по умолчанию для веба
		...
		#endif
	#endif
	//#define DEBUG_RO_FLOW				100	// отладка датчика расхода питьевого фильтра, литры в час
...
//	#define TANK_EMPTY			8        // Емкость пуста (INP6)
   ...
   // Исполнительные устройства (реле и сухие контакты) ------------------------------------------------------------------
	...
	#define LEAKAGE_TANK_RESTART_TIME	32766	// Проверка бака на утечку, для ошибки - уменьшения уровня бака на TankLeakagePercent должен произойти раньше, чем это время (32767 - выкл), сек
