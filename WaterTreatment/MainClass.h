...
struct type_option {
	...
	uint8_t  TankCheckPercent;		// десятые %, Проверка на утечку/заполняемость бака, тревога, если уровень уменьшится без потребителей на %, либо заполнение бака идет медленно (FillingTankTimeout)
...
public:
	...
	bool clear_filter_counter(char *f);				// сбросить счетчик фильтра, после нужно вызвать сохранение настроек!
